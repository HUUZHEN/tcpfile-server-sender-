    ...
    fileNameSize(0)
{
    ...
    startClientButton->setEnabled(false); // 預設為不可用
...
void TcpFileTransfer::startServer()
{
    ...
    if (!tcpServer.listen(QHostAddress::Any, port)) {
        QMessageBox::critical(this, "錯誤", "無法啟動伺服器：" + tcpServer.errorString());
        return;
    }
    serverStatusLabel->setText("伺服器正在監聽...");
}
...
void TcpFileTransfer::updateServerProgress()
{
    ...
    if (fileName.isEmpty() && tcpServerConnection->bytesAvailable() >= fileNameSize) {
        in >> fileName;
    ...
    if (tcpServerConnection->bytesAvailable() > 0) {
    ...
    if (byteReceived == totalBytes) {
        localFile->close();
        serverStatusLabel->setText("檔案接收完成");
    }
}
...
void TcpFileTransfer::displayServerError(QAbstractSocket::SocketError socketError)
{
    ...
    QMessageBox::critical(this, "伺服器錯誤", tcpServer.errorString());
}
